...
const CreateQuizQuestion = ({navigation}) => {
    ...
    const handleSubmit = async () => {
        ...
        if (!question || answers.length < 2 || !answers.some((answer) => answer.isCorrect)) {
            Alert.alert('Validation Error', 'Please enter a valid question and answers.');
            return;
        }
        ...
        try {
            ...
            if (response.data.status === 'ok') {
                ...
                Alert.alert('Error', 'Failed to create quiz question. Please try again later.');
            }
        } catch (error) {
            // Handle network error or other exceptions here
            setLoading(false);
            console.error('Error creating quiz question:', error);
            Alert.alert('Error', 'An error occurred while creating the quiz question.');
        }
    };
